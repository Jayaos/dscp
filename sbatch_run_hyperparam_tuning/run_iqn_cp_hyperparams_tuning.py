...
from sbatch_run_hyperparam_tuning.common import (
    choose_sequence_key,
...
def _build_model(config, dim_feature: int):
    ...
    return model, train_loss_fn, model_type
...
def main():
    ...
    sequence_key = choose_sequence_key(cpd.dataset, args.sequence_key, args.sequence_index)
    ...
    for trial_index, (trial_config, grid_values) in enumerate(iter_grid_configs(base_config, grid), start=1):
        ...
        result = _run_single_trial(trial_config, cpd.dataset[sequence_key], cpd.data[sequence_key])
        record = {
